...
def connection(layer_from, layer_to):
    x = tf.add(layer_from, layer_to)
    return x
...
def convolution(input, num_output):
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    conv_1x1 = convolution(vgg_layer7_out, num_classes)
    output = upsample(conv_1x1, num_classes, 5, 2)

    vgg_layer4_out_conv = convolution(vgg_layer4_out, num_classes)
    output = connection(output,vgg_layer4_out_conv)
    output = upsample(output, num_classes, 5, 2)

    vgg_layer3_out_conv = convolution(vgg_layer3_out, num_classes)
    output =  connection(output,vgg_layer3_out_conv)
    output = upsample(output , num_classes, 16, 8)
    return output
